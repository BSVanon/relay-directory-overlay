...
const _usedSignatures = new Map() // signature hex → timestamp
  ...
  return {
    ...
    verify (req, bodyString) {
      ...
      try {
        ...
        // Replay prevention: reject reused signatures
        if (_usedSignatures.has(signature)) {
          return { valid: false, reason: 'replayed signature' }
        }
        _usedSignatures.set(signature, Date.now())

        // Periodic cleanup of expired nonces
        if (_usedSignatures.size > 1000) {
          const cutoff = Date.now() - NONCE_TTL_MS
          for (const [sig, ts] of _usedSignatures) {
            if (ts < cutoff) _usedSignatures.delete(sig)
          }
        }
...
/**
 * Clear used signatures (for testing).
 */
export function clearUsedSignatures () {
