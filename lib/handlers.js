...
export async function handleSubmit (req, res, topicManager, sync = null) {
  ...
  for (const idx of outputIndexes) {
    ...
    if (result.admitted) {
      await topicManager.admit(result.entry)
 ...
 * Includes overlay metadata fields beyond the base BRC-36 spec.
 *
 * PHASE 1 LIMITATION: Full BRC-36 requires outputScript, rawTx, and
 * envelope fields (inputs/proof). We return txid, vout, satoshis plus
 * overlay-specific metadata. Full BRC-36 hydration (including rawTx and
 * SPV proof) is deferred to Phase 2 when the store retains full tx data.
 */
function toBrc36 (entry) {
  return {
    // BRC-36 base fields (partial — see limitation above)
    txid: entry.txid,
